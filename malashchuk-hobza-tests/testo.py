...
def tcp_sigint_and_bye(tester):
    ...
    # Expect BYE message
    message = tester.receive_message()
    assert message == "BYE FROM unknown\r\n", "Incoming message does not match expected BYE message after SIGINT."
...
@testcase
def tcp_too_long_msg_minus_one(tester):
    """Test that the program handles a too long TCP message correctly."""
    tester.start_server("tcp", 4567)
    tester.setup(args=["-t", "tcp", "-s", "localhost", "-p", "4567"])
    tester.send_message(('a' * 60033) + '\r\n')

    sleep(0.2)

    stdout = tester.get_stdout()
    assert any(
        ["ERROR: only messages of types BYE, ERR, MSG and REPLY are expected to be received from the server." == line for line in stdout.split("\n")]
    ), "Output does not match expected error message."
    # assert any(
    #     "ERROR: " in line for line in stdout.split("\n")
    # ), "Output does not contain expected 'ERROR: ' message."
    ...
    message = tester.receive_message()
    assert (
            message == "ERR FROM unknown IS only messages of types BYE, ERR, MSG and REPLY are expected to be received from the server.\r\n"
    ), "Incoming message does not match expected ERR message."
...
# stdout = tester.get_stdout()
    ...
    #     print(repr(line))
    # assert any(
    #     ["ERROR: too long message from server." == line for line in stdout.split("\n")]
    # ), "Output does not match expected error message."

@testcase
def tcp_too_long_server_msg(tester):
    """Test that the program handles a too long TCP message correctly."""
    tester.start_server("tcp", 4567)
    tester.setup(args=["-t", "tcp", "-s", "localhost", "-p", "4567"])
    tester.send_message(('a' * 60034) + '\r\n')
    ...
    message = tester.receive_message()
    assert (
            message == "ERR FROM unknown IS too long message from server.\r\n"
    ), "Incoming message does not match expected ERR message."

@testcase
def tcp_too_long_server_msg_plus_one(tester):
    """Test that the program handles a too long TCP message correctly."""
    tester.start_server("tcp", 4567)
    tester.setup(args=["-t", "tcp", "-s", "localhost", "-p", "4567"])
    tester.send_message(('a' * 60035) + '\r\n')
...
@testcase
def tcp_too_long_server_msg_plus_one_without_end(tester):
    """Test that the program handles a too long TCP message correctly."""
    tester.start_server("tcp", 4567)
    tester.setup(args=["-t", "tcp", "-s", "localhost", "-p", "4567"])
    tester.send_message(('a' * 60036))
...
@testcase
def tcp_auth_reply_timeout(tester):
    """Helper function to test the AUTH command followed by a Action successful reply."""
    tester.start_server("tcp", 4567)
    tester.setup(args=["-t", "tcp", "-s", "localhost", "-p", "4567"])
    ...
    stdout = tester.get_stdout()
    assert any(
        ["ERROR: waited too long for the server's reply." == line for line in stdout.split("\n")]
    ), "Output does not match expected error message."

    message = tester.receive_message()
    assert (
            message == "ERR FROM c IS waited too long for the server's reply.\r\n"
    ), "Incoming message does not match expected ERR message."

@testcase
def tcp_join_reply_timeout(tester):
    """Helper function to test the AUTH command followed by a Action successful reply."""
    tester.start_server("tcp", 4567)
    tester.setup(args=["-t", "tcp", "-s", "localhost", "-p", "4567"])
    ...
    assert any(
        ["Action Success: takjo brasko ale bud hodnej" in line for line in stdout.split("\n")]
    ), "Output does not match expected 'Action Success: takjo brasko ale bud hodnej' output."

    # Send AUTH command
    tester.execute("/join abc1")

    message = tester.receive_message()
    assert (
            message == "JOIN abc1 AS c\r\n"
    ), "Incoming message does not match expected ERR message."
...
@testcase
def tcp_invalid_message_type(tester):
    """Helper function to test the AUTH command followed by a Action successful reply."""
    tester.start_server("tcp", 4567)
    tester.setup(args=["-t", "tcp", "-s", "localhost", "-p", "4567"])
    tester.send_message("invalidmsgtype\r\n")
    sleep(0.2)
    message = tester.receive_message()
    assert (
            message == "ERR FROM unknown IS only messages of types BYE, ERR, MSG and REPLY are expected to be received from the server.\r\n"
...
@testcase
def tcp_unexpected_message_type(tester):
    """Helper function to test the AUTH command followed by a Action successful reply."""
    tester.start_server("tcp", 4567)
    tester.setup(args=["-t", "tcp", "-s", "localhost", "-p", "4567"])
    tester.send_message("JOIN\r\n")
    sleep(0.2)
    message = tester.receive_message()
    assert (
            message == "ERR FROM unknown IS only messages of types BYE, ERR, MSG and REPLY are expected to be received from the server.\r\n"
...
@testcase
def tcp_auth_reply_delay(tester):
    """Helper function to test the AUTH command followed by a Action successful reply."""
    tester.start_server("tcp", 4567)
    tester.setup(args=["-t", "tcp", "-s", "localhost", "-p", "4567"])
    ...
    tester.send_message("RepLy Nok iS something wrong has happened.\r\n")
    ...
    stdout = tester.get_stdout()
    assert any(
        ["Action Failure: something wrong has happened." == line for line in stdout.split("\n")]
    ), "Output does not match expected error message."

@testcase
def tcp_join_reply_delay(tester):
    """Helper function to test the AUTH command followed by a Action successful reply."""
    tester.start_server("tcp", 4567)
    tester.setup(args=["-t", "tcp", "-s", "localhost", "-p", "4567"])
...
@testcase
def tcp_processing_server_messages_while_joining(tester):
    """Helper function to test the AUTH command followed by a Action successful reply."""
    tester.start_server("tcp", 4567)
    tester.setup(args=["-t", "tcp", "-s", "localhost", "-p", "4567"])
    ...
    tester.send_message("MSG FROM server1 is msg from server 1.\r\n")
    tester.send_message("MSG FROM server2 is msg from server 2.\r\n")
    sleep(2.5)
    tester.send_message("MSG FROM server3 is msg from server 3.\r\n")
    sleep(1.8)
    tester.send_message("RepLy oK iS something is something :)))).\r\n")
    sleep(0.2)

    # Check the output, should contain "Action Success: vsechno cajk"
    stdout = tester.get_stdout()

    # Expected output string (dedented to remove leading spaces)
    expected_output = textwrap.dedent("""\
        server1: msg from server 1.
        server2: msg from server 2.
        server3: msg from server 3.
        Action Success: something is something :)))).
    """)
    ...
    assert stdout_clean == expected_clean, f"\nExpected:\n{expected_clean}\n\nGot:\n{stdout_clean}"

@testcase
def tcp_all_good_after_auth_reply_delay(tester):
    """Helper function to test the AUTH command followed by a Action successful reply."""
    tester.start_server("tcp", 4567)
    tester.setup(args=["-t", "tcp", "-s", "localhost", "-p", "4567"])
    ...
    # Send AUTH command
    tester.execute("/auth x y z")

    # Expect the auth message to be received by the server
    message = tester.receive_message()
    assert (
            message == "AUTH x AS z USING y\r\n"
    ), "Incoming message does not match expected AUTH message."

    tester.send_message("RepLy oK iS allright\r\n")
    ...
    stdout = tester.get_stdout()
    assert any(
        ["Action Success: allright" == line for line in stdout.split("\n")]
    ), "Output does not match expected error message."

    tester.send_message("MSG from user1234 Is elchin krasavs kawasaki\r\n")
    ...
    stdout = tester.get_stdout()
    assert any(
        ["user1234: elchin krasavs kawasaki" == line for line in stdout.split("\n")]
    ), "Output does not match expected error message."

@testcase
def tcp_all_good_after_join_reply_delay(tester):
    """Helper function to test the AUTH command followed by a Action successful reply."""
    tester.start_server("tcp", 4567)
    tester.setup(args=["-t", "tcp", "-s", "localhost", "-p", "4567"])
    ...
    tester.send_message("RepLy OK iS something wrong has happened.\r\n")
    ...
    stdout = tester.get_stdout()
    assert any(
        ["Action Success: something wrong has happened." == line for line in stdout.split("\n")]
    ), "Output does not match expected error message."


    tester.execute("/join someshi")

    # Expect the auth message to be received by the server
    message = tester.receive_message()
    assert (
            message == "JOIN someshi AS c\r\n"
    ), "Incoming message does not match expected AUTH message."
